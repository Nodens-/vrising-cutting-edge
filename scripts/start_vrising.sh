...
# Uncomment for full WINE debug output
...
# Install BepInEx
if [ "$V_RISING_INSTALL_MOD_SUPPORT" == "1" ]; then
	if [ ! -d /steamcmd/vrising/BepInEx ]; then
		curl -L -o /steamcmd/vrising/BepInEx_UnityIL2CPP_x64_6.0.0-pre.1.zip https://github.com/BepInEx/BepInEx/releases/download/v6.0.0-pre.1/BepInEx_UnityIL2CPP_x64_6.0.0-pre.1.zip
		unzip -o /steamcmd/vrising/BepInEx_UnityIL2CPP_x64_6.0.0-pre.1.zip -d /steamcmd/vrising
		rm -f /steamcmd/vrising/BepInEx_UnityIL2CPP_x64_6.0.0-pre.1.zip
	else
		echo "BepinEx seems to be installed."
	fi
fi
...
if [ "$V_RISING_APPLY_CUSTOM_SETTINGS" = "1" ]; then
	...
	jq '.Port |= env.V_RISING_SERVER_GAME_PORT|tonumber' "${V_RISING_SERVER_CONFIG_FILE}" > "/tmp/ServerHostSettings.json.tmp" 2> /dev/null && cp -f "/tmp/ServerHostSettings.json.tmp" "${V_RISING_SERVER_CONFIG_FILE}"
	jq '.QueryPort |= env.V_RISING_SERVER_QUERY_PORT|tonumber' "${V_RISING_SERVER_CONFIG_FILE}" > "/tmp/ServerHostSettings.json.tmp" 2> /dev/null && cp -f "/tmp/ServerHostSettings.json.tmp" "${V_RISING_SERVER_CONFIG_FILE}"
	jq '.MaxConnectedUsers |= env.V_RISING_SERVER_MAX_CONNECTED_USERS|tonumber' "${V_RISING_SERVER_CONFIG_FILE}" > "/tmp/ServerHostSettings.json.tmp" 2> /dev/null && cp -f "/tmp/ServerHostSettings.json.tmp" "${V_RISING_SERVER_CONFIG_FILE}"
	jq '.MaxConnectedAdmins |= env.V_RISING_SERVER_MAX_CONNECTED_ADMINS|tonumber' "${V_RISING_SERVER_CONFIG_FILE}" > "/tmp/ServerHostSettings.json.tmp" 2> /dev/null && cp -f "/tmp/ServerHostSettings.json.tmp" "${V_RISING_SERVER_CONFIG_FILE}"
	...
	jq '.ListOnMasterServer |= env.V_RISING_SERVER_LIST_ON_MNASTER_SERVER|test("true")' "${V_RISING_SERVER_CONFIG_FILE}" > "/tmp/ServerHostSettings.json.tmp" 2> /dev/null && cp -f "/tmp/ServerHostSettings.json.tmp" "${V_RISING_SERVER_CONFIG_FILE}"
	jq '.AutoSaveCount |= env.V_RISING_SERVER_AUTO_SAVE_COUNT|tonumber' "${V_RISING_SERVER_CONFIG_FILE}" > "/tmp/ServerHostSettings.json.tmp" 2> /dev/null && cp -f "/tmp/ServerHostSettings.json.tmp" "${V_RISING_SERVER_CONFIG_FILE}"
	jq '.AutoSaveInterval |= env.V_RISING_SERVER_AUTO_SAVE_INTERVAL|tonumber' "${V_RISING_SERVER_CONFIG_FILE}" > "/tmp/ServerHostSettings.json.tmp" 2> /dev/null && cp -f "/tmp/ServerHostSettings.json.tmp" "${V_RISING_SERVER_CONFIG_FILE}"
...
if [ "$V_RISING_SERVER_UPDATE_MODE" = "1" ]; then
	/usr/local/libexec/jobberrunner /home/vrserver/.jobber &
fi

# Construct a Windows/Wine path for the persistent data
V_RISING_SERVER_PERSISTENT_DATA_PATH_WINE="Z:$V_RISING_SERVER_PERSISTENT_DATA_PATH"
...
xvfb-run -d \
  /bin/bash -c 'WINEARCH=win64 WINEDLLOVERRIDES="winhttp=n,b" wine64 /steamcmd/vrising/VRisingServer.exe "${V_RISING_SERVER_STARTUP_COMMAND}"' &
